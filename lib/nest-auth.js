...
async function fetchAccessToken(issueToken, cookie, { timeoutMs = 15000 } = {}) {
  ...
  if (!body.access_token) {
    throw new NestAuthError(
      `Google returned no access token${body.error ? `: ${body.error}` : ''}`,
  ...
  const merged = typeof response.headers.getSetCookie === 'function'
    ? mergeCookies(cookie, response.headers.getSetCookie())
    : cookie;
  ...
  return {
    ...
    // nytt fra DevTools.
    cookie: merged.includes(`${REQUIRED_COOKIE}=`) ? merged : cookie,
